...
type funcionarioRepository struct {
	db        *sql.DB
	pontoRepo ponto.PontoRepository
}

func NovoFuncionarioRepository(conn *database.SQLStr, pontoRepo ponto.PontoRepository) FuncionarioRepository {
...
func (r *funcionarioRepository) ListarFuncionarios(page int) ([]models.Funcionario, error) {
	...
	query := `
		SELECT 
			...
			DateAdmissao, HoraEntrada, HoraSaida, ValeTransporteSemanal, 
			Status, ChavePix, Salario, Admin
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			...
			&f.DateAdmissao, &f.HoraEntrada, &f.HoraSaida, &f.ValeTransporteSemanal,
			&f.Status, &f.ChavePix, &f.Salario, &f.Admin,
		); err != nil {
...
func (r *funcionarioRepository) BuscarFuncionarioPorCPF(cpf string) (*models.FuncionarioComPontos, error) {
	...
	ponto, err := r.pontoRepo.ListarPontosPorIdEDia(funcionario.Id, dia)
	if err != nil {
		return nil, fmt.Errorf("erro ao buscar pontos para o funcionário %d: %w", funcionario.Id, err)
	}

	funcionarioComPontos.Pontos = append(funcionarioComPontos.Pontos, ponto)

	switch {
	case ponto.HManha.Valid && ponto.HAlmocoSaida.Valid && ponto.HAlmocoRetorno.Valid && ponto.HNoite.Valid:
		funcionarioComPontos.BotaoStatus = "Todos os pontos registrados"
	case ponto.HManha.Valid && !ponto.HAlmocoSaida.Valid:
		funcionarioComPontos.BotaoStatus = "Pausa"
	case ponto.HAlmocoSaida.Valid && !ponto.HAlmocoRetorno.Valid:
		funcionarioComPontos.BotaoStatus = "Retorno"
	case ponto.HAlmocoRetorno.Valid && !ponto.HNoite.Valid:
		funcionarioComPontos.BotaoStatus = "Saída"
	default:
...
func (r *funcionarioRepository) EditarFuncionario(funcionario *models.Funcionario) error {
	...
	if funcionario.Senha != "" {
		hashedPassword := sha256.Sum256([]byte(funcionario.Senha))
		authUpdate := `
			UPDATE AUTH SET password = @Password WHERE username = @Username
		`
