...
func NovoDocumentoRepository(conn *database.SQLStr) DocumentoRepository {
	return &documentoRepository{
		db: conn.DB(),
	}
}

func (r *documentoRepository) CriarDocumento(doc *Documento) error {
	var existe bool
...
func (r *documentoRepository) ListarDocumentos(page int) ([]Documento, error) {
	var docs []Documento
	limit := 10
	offset := (page - 1) * limit
	...
	query := `
		SELECT 
			...
			d.Descricao
		FROM 
			Documentos d WITH (NOLOCK)
		JOIN 
			Funcionarios f WITH (NOLOCK) ON d.IdFuncionario = f.Id
		ORDER BY 
			d.Data_Criacao DESC
		OFFSET @Offset ROWS
...
func (r *documentoRepository) FiltrarDataDocumento(inicioData, fimData time.Time) ([]Documento, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("erro ao consultar documentos por data: %w", err)
	}
	defer rows.Close()
